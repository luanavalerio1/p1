...
export function CameraScreen({ navigation }: TabTypes) {
  ...
  function viraCamera() {
    setType((cur) =>
      cur === CameraType.back ? CameraType.front : CameraType.back
    );
  }
  async function tiraPhoto() {
    if (ref.current) {
      const picture = await ref.current.takePictureAsync();
      // Se no navigation mudar o nome da página, tem que mudar aqui também!
  ...
  return (
    <View style={styles.container}>
      ...
      <Camera
        ...
        type={type}
        ref={ref}
        // Se não for detecção de rosto pode apagar desde o "onFacesDetected" até o "tracking" (inclusive as chaves)
      ...
      {/* Para ler o QRCode novamente (Se não for de QrCode pode apagar) */}
      {scanned && (
      ...
      <View style={styles.sorriso}>
        {
          /* Para mudar o que vai ser detectado (Se não for detecção de rosto pode apagar) */ face &&
